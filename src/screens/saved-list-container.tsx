...
interface SavedListContainerProps {
  type: ListType;
  showHeader?: boolean;
  title?: string;
}
...
const SavedListContainer = ({
  type,
  showHeader = false,
  title = "Wishlist",
}: SavedListContainerProps) => {
  const [selectType, setSelectType] = useState<"movie" | "tv">("movie");

  const favoriteMoviesQuery = useGetFavoriteMovies();
  const favoriteTvQuery = useGetFavoriteTvShows();
  const watchlistMoviesQuery = useGetWatchlistMovies();
  const watchlistTvQuery = useGetWatchlistTvShows();

  const moviesQuery = type === "favorite" ? favoriteMoviesQuery : watchlistMoviesQuery;
  const tvQuery = type === "favorite" ? favoriteTvQuery : watchlistTvQuery;
